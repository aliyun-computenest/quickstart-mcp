...
- 模板不提供 API 转 MCP。
- 模板只注册计算巢实例自身部署的 MCP。
- 用户在 AI 网关控制台创建的 HTTP 转 MCP 可以从配置页“网关接入”页签接入到当前计算巢服务实例。
- 网关接入项写入独立参数 `ManagedMcpConfigJson`，进入实例 MCP 清单，但不进入 FC/ACS 注册逻辑，也不重复创建 MCP Server。
- 网关接入项的真实状态以 AI 网关为准；服务实例详情和 Agent 保存前需要重新查询网关并对账，失效项只从计算巢侧移出，不自动重建或删除 AI 网关资源。
- Agent 部署页按服务实例展示统一的可用 MCP 清单，不区分 MCP 来源和创建方式。

推荐引导文案：

> 如需使用已在 AI 网关中创建的 MCP，请在当前服务实例的“配置 MCP”弹窗中切换到“网关接入”，选择目标 MCP 后接入当前实例。接入不会重新注册 MCP Server。
